...
function pad(){
 ...
 while(h&&h!==document.body){var r=h.getBoundingClientRect();if(r.width>=window.innerWidth-4&&r.top<=4&&r.height>=36&&r.height<=160){
   if(!h.hasAttribute('data-kbn-pad')){h.setAttribute('data-kbn-pad','1');h.style.paddingRight=((parseFloat(getComputedStyle(h).paddingRight)||0)+124)+'px';}
...
document.addEventListener('mousedown',function(e){if(e.button)return;if(e.clientY>44)return;if(e.clientX>window.innerWidth-124)return;if(e.target.closest('button,a,input,select,textarea,[role=button],[contenteditable],svg'))return;var k=w();if(k)k.startDragging();},true);
...
/* This window hosts a single office sub-module (captured from the launch URL).
   The office settings page scopes itself NATIVELY when opened with ?app=<sub>
   (hides other sub-modules' tabs, breadcrumb back to /office/<sub>) — so we just
   append ?app= whenever the SPA reaches the global settings route. No DOM hacks. */
...
function scopeSettings(){
 if(!KBN_SUB)return;
 if(location.pathname==='/office/settings'&&location.search.indexOf('app=')<0){
  history.replaceState({},'','/office/settings?app='+KBN_SUB);
...
async fn open_document(
    ...
    WebviewWindowBuilder::new(&app, &label, WebviewUrl::External(parsed))
        ...
        .decorations(false)
        // Match the main window: borderless windows must be transparent + shadowed,
        // otherwise WebView2 lagging behind a resize exposes the raw (black) HWND
